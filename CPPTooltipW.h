/*
	CPPToolTipW.h
	Classe per la gestione del tooltip (normale, notificazione, balloon).
	Il codice orginale e' la famosa triada CPPToolTip, CPPHtmlDrawer e CPPDrawManager di
	Eugene Pustovoyt, qui modificata e riadattata.

	Vedi le note in CPPToolTipW.cpp.

	Note modifiche:

	su CPPToolTip.h (originale)
	- aggiunto: headers (#include)
	- rimosso:	#define PPTOOLTIP_USE_MENU
	- cambiato: in AddTool() cambiato passaggio parametro CSize da referenza (&) a valore (copia)
	- cambiato:	BOOL Create(CWnd* pParentWnd, BOOL bBalloon = TRUE); in BOOL Create(HWND hWndParent, BOOL bBalloon = TRUE);

	Luca Piergentili, 2005

	su CPPToolTip.h modificato (vedi sopra) a CPPToolTipW.h
	- aggiunto temporalmente e rimosso: ShowHelpTooltipW()
	- aggiunti membro e dati per gestire il click sul balloon esteso
	- trasformazione a Unicode esplicito, non viene piu' specificato il tag //LPI perche' viene rivisto il codice intero
...
//LPI lo usa in CPPHtmlDrawer, ma non qui...
#ifdef _MFC_VER
	#define CPPString	CStringW //MFC program
#else
	#include "StdString.h"
	#ifdef _UNICODE
...
//LPI #define PPTOOLTIP_USE_MENU

#define PPTOOLTIP_CLASSNAME    L"CPPToolTipW"  // Window class name
...
typedef struct tagPPTOOLTIP_INFO
{
	...
	CPPString	sTooltip;		// The string of the tooltip
	UINT        nMask;			// The mask 
...
class CPPToolTipW : public CWnd
{
//	friend BOOL CALLBACK EnumChildWinF(HWND hwnd, LPARAM lParam);
// Construction
public:
//LPI BOOL Create(CWnd* pParentWnd, BOOL bBalloon = TRUE);
...
protected:
	...
	CPPHtmlDrawerW m_drawer; //HTML drawer object
...
//LPI
public:
	// metodo per impostare la notifica del click
	void SetOnBalloonClick(HWND hWndNotify,UINT nMsgNotify)
	{
		m_hWndClickNotify = hWndNotify;
		m_nMsgClickNotify = nMsgNotify;
	}
protected:
	HWND m_hWndClickNotify = NULL;	// finestra a cui inviare il messaggio
	UINT m_nMsgClickNotify = 0;		// ID del messaggio custom (es. WM_USER + X)
//LPI
...
public:
	...
	void AddTool(CWnd * pWnd, LPCWSTR lpszString = NULL, LPCRECT lpRectBounds = NULL, DWORD dwIDTool = 0);
	...
	void AddTool(CWnd * pWnd, LPCWSTR lpszString, HICON hIcon, LPCRECT lpRectBounds = NULL, DWORD dwIDTool = 0);
	void AddTool(CWnd * pWnd, LPCWSTR lpszString, DWORD dwIdIcon, CSize szIcon = CSize(0, 0), LPCRECT lpRectBounds = NULL, DWORD dwIDTool = 0);
	void AddTool(CWnd * pWnd, LPCWSTR lpszString, DWORD dwIdBitmap, COLORREF crMask, CSize szBitmap = CSize(0, 0), LPCRECT lpRectBounds = NULL, DWORD dwIDTool = 0);
	...
	void UpdateTipText(LPCWSTR lpszText, CWnd * pWnd, DWORD dwIDTool = 0);
	...
	void ShowHelpTooltip (LPPOINT pt, LPCWSTR lpszString, HICON hIcon = NULL);
	void ShowHelpTooltip (LPPOINT pt, LPCWSTR lpszString, DWORD dwIdIcon, CSize szIcon = CSize(0, 0));
	void ShowHelpTooltip (LPPOINT pt, PPTOOLTIP_INFO & ti);
	void HideTooltip();

	//LPI
	//void ShowHelpTooltipW(LPPOINT pt, LPCWSTR lpcwszString, DWORD dwIdIcon, CSize szIcon = CSize(0, 0));
	//void ShowHelpTooltipW(LPPOINT pt, LPCWSTR lpcwszString, HICON hIcon = NULL);
	//LPI
...
//	void EnableTextWrap(BOOL bEnable = TRUE){m_drawer.EnableTextWrap(bEnable);};
	...
	void SetTextStyle(LPCWSTR lpszStyleName, LPCWSTR lpszStyleValue);
	void SetCssStyles(LPCWSTR lpszCssStyles = NULL);
	void SetCssStyles(DWORD dwIdCssStyle, LPCWSTR lpszPathDll = NULL); //Sets the CSS styles
	LPCWSTR GetCssStyles();
	...
	static LPCWSTR GetVersionC()	{return (LPCWSTR)L"2.2 beta";}
...
protected:
	...
	void FreeResources();
	CPPString GetDebugInfoTool(LPPOINT lpPoint);
	...
	CPPString GetMaxDebugString(LPCWSTR lpszText);
	CPPString CreateDebugCell(CPPString sTitle, LPCWSTR lpszDescription);
