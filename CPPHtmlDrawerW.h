...
public:
	//Drawing methods
	void  Draw(HDC hDC, LPCWSTR lpszHtml, LPPOINT lpPoint);
	void  PrepareOutput(HDC hDC, LPCWSTR lpszHtml, LPSIZE lpSize); //Prepares to draw the HTML string
	...
	void SetCssStyles(LPCWSTR lpszCssString = NULL); //Sets the CSS styles
	void SetCssStyles(DWORD dwIdCssString, LPCWSTR lpszPathDll = NULL); //Sets the CSS styles
	LPCWSTR GetCssStyles(); //Returns the current CSS styles
	...
	void LoadResourceDll(LPCWSTR lpszPathDll, DWORD dwFlags = 0); //Sets the path to the resource's DLL
...
//	BOOL IsTextWrapEnabled() {return m_bIsTextWrapEnabled;};
	...
	CPPDrawManagerW * GetDrawManager();

	static short GetVersionI()		{return 0x13;}
	static LPCWSTR GetVersionC()	{return (LPCWSTR)L"1.3 beta";}
...
//	int m_nLineDescent;
	...
	LOGFONTW m_lfDefault; //Default font
	HFONT m_hFont;
...
protected:
	...
	void AddTagToList(LPCWSTR lpszName, DWORD dwTagIndex, LPCWSTR lpszFullName); //Add tag to the list of tags
	DWORD GetTagFromList(CPPString sTagName, CPPString & strFullName, BOOL & bCloseTag); //Get tag from the list

	void SetListSpecChars();
	void AddSpecChar(LPCWSTR lpszAlias, wchar_t tch);
	void AddSpecChar(LPCWSTR lpszAlias, LPCWSTR lpszValue);
	...
	HICON GetIconFromFile(LPCWSTR lpszPath, int nWidth = 0, int nHeight = 0) const; //Load an icon from the file
	HICON GetIconFromDll(DWORD dwID, int nWidth = 0, int nHeight = 0, LPCWSTR lpszPathDll = NULL) const; //Load an icon from the dll resources
	HBITMAP GetBitmapFromResources(DWORD dwID) const; //Load a bitmap from the app resources
	HBITMAP GetBitmapFromFile(LPCWSTR lpszPath) const; //Load a bitmap from the file
	HBITMAP GetBitmapFromDll(DWORD dwID, LPCWSTR lpszPathDll = NULL) const; //Load a bitmap from the dll resources
	CPPString GetStringFromResource(DWORD dwID) const; //Load a string from the app resources
	CPPString GetStringFromDll(DWORD dwID, LPCWSTR lpszPathDll = NULL) const; //Load a string from the dll resources
...
//public:
	...
	LPLOGFONTW GetSystemToolTipFont() const; //Gets the system logfont
	...
	HINSTANCE GotoURL(LPCWSTR url, int showcmd = SW_SHOW);
	LONG GetRegKey(HKEY key, LPCWSTR subkey, LPWSTR retdata);

	//Functions for notify
	void CallbackOnClickHyperlink(LPCWSTR sLink);
	...
	void SelectNewHtmlStyle(LPCWSTR lpszNameStyle, STRUCT_CHANGESTYLE & cs);
	...
	void SetColorName(LPCWSTR lpszColorName, COLORREF color);
	COLORREF GetColorByName(LPCWSTR lpszColorName, COLORREF crDefColor = RGB(0, 0, 0));

	BOOL GetIndexNextAlphaNum(CPPString & str, int & nIndex, BOOL bArithmetic = FALSE);
	BOOL GetBeginParameter(CPPString & str, int & nIndex, wchar_t chSeparator = L':');
	wchar_t GetIndexNextChars(CPPString & str, int & nIndex, CPPString strChars);
	wchar_t GetIndexNextNoChars(CPPString & str, int & nIndex, CPPString strChars);
	CPPString GetParameterString(CPPString & str, int & nIndex, wchar_t chBeginParam = L':', CPPString strSeparators = L";");
	CPPString GetNameOfTag(CPPString & str, int & nIndex);
	CPPString GetWordWrap(CPPString & str, int nMaxSize, int & nRealSize);

	//Functions for the map of the styles
	LPCWSTR GetTextStyle(LPCWSTR lpszStyleName);
	void RemoveTextStyle(LPCWSTR lpszStyleName);
	void AddToTextStyle(LPCWSTR lpszStyleName, LPCWSTR lpszAddStyle);
	...
	int GetCountOfChars(CPPString str, wchar_t tchar = L' '); //Gets counts of chars
};
